...
m = glm(Status~wt.loss+Sex+AgeGroup,
        data=lung,
        family='binomial')
rm_mvsum(model=m,data = lung,p.adjust='holm')
```
...
#summary(fm1BW.lme)
rm_mvsum(fm1BW.lme,data=BodyWeight)
```
...
# For lm
response <- 'wt.loss'
lm_ecog <-lm(wt.loss~ph.ecog,data=lung) 
m2 <- lm_ecog
lm_null <- lm(paste0(response,'~1'),data=m2$model)
lrt_p <- as.vector(na.omit(anova(lm_null,m2)[,"Pr(>F)"]))


# for coxph
lrt_p <- summary(m2)$logtest['pvalue']

# for crr - uses estimating equations, so leave as is

# for glm
lung <- lung %>%
  mutate(ECOG=factor(ph.ecog))
glm_ecog <- glm(Status~ECOG,data=lung,family='binomial')
glm_null <- update(glm_ecog,formula='Status~1',data=glm_ecog$model)
lrt_p <- as.vector(na.omit(anova(glm_null,glm_ecog,test="LRT")[,"Pr(>Chi)"]))
