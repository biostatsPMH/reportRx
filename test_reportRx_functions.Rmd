...
## Comments from Katherine to fix
...
lung$Sex <- factor(lung$Sex, levels=c("","Empty level","Male","Female"))

#linear model: no issue
summary(lm(time~Sex, data=lung))
rm_uvsum(response="time",covs="Sex",data=lung)
...
rm_uvsum(data=lung, response=c("time","status"),covs="Sex")
...
summary(glm(binaryvar~Sex, data=lung, family=binomial(link="logit")))
rm_uvsum(data=lung, response="binaryvar",covs="Sex")

```


## Comment from Osvaldo

rm_covsum and pcovsum throw an error if maincov=NULL and full=FALSE

**fixed**

```{r}
rm_covsum(data=lung,
       covs=c('Sex','age'),full=FALSE)
