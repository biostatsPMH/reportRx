...
rm_uvsum(response = c('time','status'),
         ...
         data=lung,
         CIwidth=.99,nicenames = F,digits=3)

uvsum(response = c('time','status'),
         covs=c('Sex','ph.ecog','meal.cal','age'),
         data=lung,
         CIwidth=.99,nicenames = F,markup = FALSE,sanitize=FALSE)
...
tab<-rm_uvsum(response = 'cyl',
         covs=c('qsec'),
         data=mtcars,
         CIwidth=.90,showN=F,tableOnly = T)
...
rm_uvsum(response = 'weight',
         ...
         data=BodyWeight,showN=T,nicenames = TRUE)
```
...
tab <- mvsum(model=m,data = lung,markup = F,sanitize = F)
...
```{r}
surv_fit <- survival::coxph(Surv(os_time,os_status)~age+sex+baseline_ctdna+l_size+change_ctdna_group,
data=pembrolizumab)
rm_mvsum(surv_fit,tableOnly = F,nicenames = T)
summary(surv_fit)$coefficients


surv_fit <- survival::coxph(Surv(os_time,os_status)~baseline_ctdna,
data=pembrolizumab)
summary(surv_fit)$coefficients
anova(surv_fit)
```
...
gee1 <- geeglm(mf, data=dietox, id=Pig, family=gaussian("identity"), corstr="ar1")
summary(gee1)
mvsum(model=gee1,data=dietox)
...
uvsumTable  <- rm_uvsum(response = 'Status',
         covs=c('Sex','wt.loss','AgeGroup'),
         data=lung,
         CIwidth=.95,
         tableOnly = T)

m <- glm(Status~Sex+wt.loss+AgeGroup,data=lung,family='binomial')
mvsumTable  <- rm_mvsum(m, tableOnly = T,showN=T)
...
tab1$Model = 'Model 1'
tab1$`Global p-value` = NA # necessary to make the columns match
tab2$Model = 'Model 2'
...
m <- lm(wt.loss~meal.cal,data=lung)
mvsumTable  <- rm_mvsum(m, tableOnly = T,showN=T)
...
data <- data.frame(x1=rnorm(N),
                   x2=rnorm(N,2,.5),
                   x3=rnorm(N,5),
                   c1=factor(sample(c('little','big'),N,T)),
                   c2=factor(sample(c('A','B','C'),N,T)))
attach(data)
data$y = .3*x1+.5*x2+.25*x3+as.numeric(c1)+2*as.numeric(c2)+Ei
data$yBin = ifelse(data$y<median(data$y),0,1)
data$z = ifelse(Ei<quantile(Ei,.2),0,1)

# LM
m1 <- lm(y~x1+x2+c1+c2,data=data)
round(summary(m1)$coefficients,4)
round(data.frame(car::Anova(m1)),4)
rm_mvsum(m1,digits=4)

# GLM
m1 <- glm(y~x1+x2+x3+c1+c2,data=data)
round(summary(m1)$coefficients,4)
round(data.frame(car::Anova(m1)),4)
#pchisq(summary(m1)$coefficients[,3]^2,1,lower.tail = F)
pf(summary(m1)$coefficients[,3]^2,1,10000,lower.tail = F)
pf(summary(m1)$coefficients[,3]^2,1,34,lower.tail = F)
rm_mvsum(m1,digits=4)

# GLM Logistic
m1 <- glm(yBin~x1+x2+x3,data=data,family='binomial')
round(summary(m1)$coefficients,4)
round(data.frame(car::Anova(m1)),4)
rm_mvsum(m1,digits=4)

# CoxPh
m1 <- coxph(Surv(y,z)~x1+x2+c1+c2,data = data)
round(summary(m1)$coefficients,4)
round(data.frame(car::Anova(m1)),4)
rm_mvsum(m1,data=data,digits=4)
